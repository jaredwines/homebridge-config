...
class DreoPlatform {
    ...
    async discoverDevices() {
        ...
        let auth = await new DreoAPI_1.default().authenticate(this, email, password, 'us');
        this.log.debug('\n\nREMOTE:\n', auth);
        ...
        if (auth.region === 'NA') {
            ...
            auth = await new DreoAPI_1.default().authenticate(this, email, password, 'eu');
            auth.server = 'eu';
        }
        else {
        ...
        const dreoDevices = await new DreoAPI_1.default().getDevices(this, auth);
        this.log.debug('\n\nDEVICES:\n', dreoDevices);
        ...
        const ws = await new DreoAPI_1.default().startWebSocket(this, auth);
        ...
        for (const device of dreoDevices) {
            ...
            const state = await new DreoAPI_1.default().getState(this, device.sn, auth);
            if (state === undefined) {
            ...
            if (existingAccessory) {
                ...
                new FanAccessory_1.FanAccessory(this, existingAccessory, state, ws, this.log);
                ...
                // this is imported from `platformAccessory.ts`
                new FanAccessory_1.FanAccessory(this, accessory, state, ws, this.log);
