...
class TplinkSmarthomePlatform {
    constructor(log, config, api) {
        ...
        if (!(0, semver_1.satisfies)(process.version, packageConfig.engines.node)) {
            this.log.error('Error: not using minimum node version %s', packageConfig.engines.node);
        }
        if (api.versionGreaterOrEqual == null ||
            !api.versionGreaterOrEqual('1.3.0')) {
            this.log.error(`homebridge-tplink-smarthome requires homebridge >= 1.3.0. Currently running: ${api.serverVersion}`);
            throw new Error(`homebridge-tplink-smarthome requires homebridge >= 1.3.0. Currently running: ${api.serverVersion}`);
        }
        this.log.debug('config.json: %j', config);
        this.config = (0, config_1.parseConfig)(config);
        this.log.debug('config: %j', this.config);
        this.customCharacteristics = (0, characteristics_1.default)(api.hap.Characteristic);
        ...
        client.on('device-new', (device) => {
            this.log.info(`Device First Online: ${chalk_1.default.blue(`[${device.alias}]`)} %s [%s]`, device.deviceType, device.id, device.host, device.port);
    ...
    getServiceName(service) {
        return (0, utils_1.lookup)(this.api.hap.Service, (thisKeyValue, value) => (0, utils_1.isObjectLike)(thisKeyValue) &&
    ...
    getCharacteristicName(characteristic) {
        ...
        if ('UUID' in characteristic) {
            return (0, utils_1.lookupCharacteristicNameByUUID)(this.api.hap.Characteristic, characteristic.UUID);
        }
        return undefined;
    }
    /**
